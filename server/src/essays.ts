...
export function registerEssayRoutes(app: Express, pool: Pool | null, authenticate: Authenticate, requireAdmin: RequireAdmin) {
  app.get('/v1/universities/:universityId/essays', async (req, res) => {
    ...
    try {
      const result = await pool.query(`
        SELECT e.id,e.university_id,e.question,e.word_limit,e.preview_text,e.author_name,e.author_avatar_url,e.created_at
  ...
  app.post('/v1/university-essays', authenticate, async (req: AuthRequest, res) => {
    ...
    try {
      ...
      const result = await pool.query(`
        INSERT INTO public.university_essays(university_id,question,word_limit,preview_text,author_name,author_avatar_url,full_text,created_by)
        VALUES($1,$2,$3,$4,$5,$6,$7,$8)
        RETURNING id,university_id,question,word_limit,preview_text,author_name,author_avatar_url,created_at`,
        [parsed.data.universityId, parsed.data.question, parsed.data.wordLimit, parsed.data.previewText, parsed.data.authorName, parsed.data.authorAvatarUrl || null, parsed.data.fullText, String(a.sub ?? '') || null]);
