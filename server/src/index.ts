...
app.use(cors({ origin: (process.env.ALLOWED_ORIGINS ?? '').split(',').map(x => x.trim()).filter(Boolean), credentials: true, methods: ['GET', 'POST'], allowedHeaders: ['Content-Type', 'Authorization'] }));
...
function requireAdmin(req: express.Request, res: express.Response): boolean {
  if (!isAdminRequest(req)) { res.status(403).json({ error: 'admin_only' }); return false; }
  return true;
}
...
const paymentSchema = z.object({
  name: z.string().trim().min(1).max(100), surname: z.string().trim().min(1).max(100),
  location: z.string().trim().min(1).max(200), phone: z.string().trim().min(7).max(30),
  serviceEmail: z.string().email().optional(), amountMinor: z.number().int().nonnegative().optional(),
  currency: z.string().trim().length(3).default('UZS'), contentId: z.string().uuid().optional(), note: z.string().trim().max(1000).optional()
});

async function ensurePaymentsTable() {
  if (!pool) return false;
  await pool.query(`CREATE TABLE IF NOT EXISTS public.payments (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(), name text NOT NULL, surname text NOT NULL, location text NOT NULL, phone text NOT NULL,
    service_email text NOT NULL, amount_minor bigint, currency text NOT NULL DEFAULT 'UZS',
    content_id uuid REFERENCES public.content_items(id) ON DELETE SET NULL,
    status text NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','paid','failed','refunded','canceled')),
    note text, created_by text NOT NULL, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now());
...
async function createPayment(req: express.Request, res: express.Response, createdBy: string) {
  ...
  try {
    await ensurePaymentsTable(); const p = parsed.data;
    const result = await pool.query(`INSERT INTO public.payments
      (name,surname,location,phone,service_email,amount_minor,currency,content_id,note,created_by)
...
app.get('/payments', requirePaymentsBrowserAuth, async (_req, res) => {
  ...
  try {
    ...
    const result = await pool.query(`SELECT id,name,surname,location,phone,service_email,amount_minor,currency,content_id,status,created_by,created_at FROM public.payments ORDER BY created_at DESC LIMIT 100`);
